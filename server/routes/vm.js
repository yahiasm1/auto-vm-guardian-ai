...
// Get VM info
router.get('/:vmName', isAuthenticated, async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      message: error.message || `Failed to get info for VM ${req.params.vmName}` 
    });
  }
});

// Start a VM
router.post('/start/:vmName', isAuthenticated, async (req, res) => {
  try {
    const { vmName } = req.params;
...
// Stop a VM (forceful)
router.post('/stop/:vmName', isAuthenticated, async (req, res) => {
  try {
    const { vmName } = req.params;
...
router.post('/shutdown/:vmName', isAuthenticated, async (req, res) => {
  try {
    const { vmName } = req.params;
...
// Delete a VM
router.delete('/:vmName', isAuthenticated, isAdmin, async (req, res) => {
  try {
    const { vmName } = req.params;
...
// Restart a VM
router.post('/restart/:vmName', isAuthenticated, async (req, res) => {
  try {
    const { vmName } = req.params;
...
// Suspend a VM
router.post('/suspend/:vmName', isAuthenticated, async (req, res) => {
  try {
    const { vmName } = req.params;
...
// Resume a VM
router.post('/resume/:vmName', isAuthenticated, async (req, res) => {
  try {
    const { vmName } = req.params;
